...
def crop_prediction():
    ...
    if request.method == 'POST':
        ...
        #override long/lat for Terry
        longitude = -84.51917
        latitude = 9.93178
        year = 2030

        #erase after funcion works
        #temperature = 80
        #rainfall = 0.8
        ...
        return render_template('results.html', prediction=final_prediction, title=title)

        #end override for test
        
        #use below when weather is functional
        if weather_fetch(long,lat,year) != None:
            temperature, rainfall = weather_fetch(long,lat,year)
            data = np.array([[N, P, K, temperature, humidity, ph, rainfall]])
            my_prediction = crop_recommendation_model.predict(data)
            final_prediction = my_prediction[0]
